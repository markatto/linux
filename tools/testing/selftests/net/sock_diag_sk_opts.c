...
/*
 * An option reads as set only when the kernel both reports the bit (it is in
 * the selector) and has it turned on (it is in the value); a bit the kernel
 * does not know about is never "set".
 */
static int opt_set(const struct nla_bitfield32 *o, __u32 bit)
{
	return (o->selector & bit) && (o->value & bit);
}
...
static int unix_sk_opts(unsigned int ino, struct nla_bitfield32 *o)
{
	struct {
		struct nlmsghdr nlh;
...
static int packet_sk_opts(unsigned int ino, struct nla_bitfield32 *o)
{
	struct {
		struct nlmsghdr nlh;
...
static int netlink_sk_opts(unsigned int ino, struct nla_bitfield32 *o)
{
	struct {
		struct nlmsghdr nlh;
...
static void test_inet_tcp(void)
{
	...
	if (inet_sk_opts(IPPROTO_TCP, sock_ino(fd), &o))
		...
		ksft_test_result(opt_set(&o, SK_DIAG_OPT_REUSEADDR) &&
				 opt_set(&o, SK_DIAG_OPT_REUSEPORT) &&
				 opt_set(&o, SK_DIAG_OPT_KEEPALIVE) &&
				 !opt_set(&o, SK_DIAG_OPT_BROADCAST) &&
				 !opt_set(&o, SK_DIAG_OPT_LINGER),
...
static void test_inet_udp_broadcast(void)
{
	...
	if (inet_sk_opts(IPPROTO_UDP, sock_ino(fd), &o))
		...
		ksft_test_result(opt_set(&o, SK_DIAG_OPT_BROADCAST) &&
				 !opt_set(&o, SK_DIAG_OPT_REUSEADDR) &&
				 !opt_set(&o, SK_DIAG_OPT_TIMESTAMP),
				 "inet UDP: broadcast\n");
	close(fd);
}
...
static void test_inet_udp_timestamp(void)
{
	...
	if (inet_sk_opts(IPPROTO_UDP, sock_ino(fd), &o))
		...
		ksft_test_result(opt_set(&o, SK_DIAG_OPT_TIMESTAMP) &&
				 !opt_set(&o, SK_DIAG_OPT_TIMESTAMPNS),
...
static void test_inet_udp_timestampns(void)
{
	...
	if (inet_sk_opts(IPPROTO_UDP, sock_ino(fd), &o))
		...
		ksft_test_result(opt_set(&o, SK_DIAG_OPT_TIMESTAMPNS) &&
				 !opt_set(&o, SK_DIAG_OPT_TIMESTAMP),
...
static void test_unix(void)
{
	...
	if (unix_sk_opts(sock_ino(fd), &o))
		...
		ksft_test_result(opt_set(&o, SK_DIAG_OPT_REUSEADDR),
				 "AF_UNIX: reuseaddr (UDIAG_SHOW_SK_OPTS)\n");
	close(fd);
}
...
static void test_netlink(void)
{
	...
	if (netlink_sk_opts(sock_ino(fd), &o))
		...
		ksft_test_result(opt_set(&o, SK_DIAG_OPT_REUSEADDR),
				 "AF_NETLINK: reuseaddr (NDIAG_SHOW_SK_OPTS)\n");
	close(fd);
}
...
static void test_packet(void)
{
	...
	if (packet_sk_opts(sock_ino(fd), &o))
		...
		ksft_test_result(opt_set(&o, SK_DIAG_OPT_BROADCAST),
				 "AF_PACKET: broadcast (PACKET_SHOW_SK_OPTS)\n");
	close(fd);
}
